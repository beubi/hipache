...
Worker.prototype.runServer = function (config) {
    ...
    var httpRequestHandler = function (req, res) {
        ...
        var proxyRequest = function () {
            this.cache.getBackendFromHostHeader(req.headers.host, function (err, code, backend) {
                ...
                proxy.web(req, res, {
                    target: backend.href,
                    xfwd: config.server.addXFwdHeaders,
                    ssl: { servername: req.headers.host },
                    secure: config.server.checkCertificates
                });
            });
        }.bind(this);
    ...
    var httpAppProxy = httpRequestHandler;
    var wsAppProxy = wsRequestHandler;

    if (config.injections !== undefined) {
        var selects = [];
        config.injections.forEach(function (injection){
            var simpleSelect = {};
            simpleSelect.query = injection.query;
            simpleSelect.func = function (item) {
                switch (injection.type) {
                    case 'end':
                        var rs = item.createReadStream();
                        var ws = item.createWriteStream({outer: false});
                        rs.pipe(ws, {end: false});
                        rs.on('end', function () {
                            ws.end(injection.content);
                        });
                        break;
                    case 'replace':
                        var ws = item.createWriteStream({outer: false});
                        ...
                        break;
                    case 'replaceOuter':
                        var ws = item.createWriteStream();
                        ws.end(injection.content);
                        break;
                    case 'start':
                        var rs = item.createReadStream();
                        var ws = item.createWriteStream({outer: false});
                        ws.write(injection.content);
                        rs.pipe(ws);
                        break;
                    case 'setAttribute':
                        item.setAttribute(injection.attribute, injection.value);
                        break;
                    case 'removeAttribute':
                        item.removeAttribute(injection.attribute);
                }
            };
            selects.push(simpleSelect);
        });
        httpAppProxy = connect();
        httpAppProxy.use(harmon([], selects));
        httpAppProxy.use(httpRequestHandler);
        wsAppProxy = connect();
        wsAppProxy.use(harmon([], selects));
        wsAppProxy.use(wsRequestHandler);
    }

    // Servers creation
    config.http.bind.forEach(function (options) {
        counter++;
        var httpServer = http.createServer(httpAppProxy);
        httpServer.on('connection', tcpConnectionHandler);
        httpServer.on('upgrade', wsAppProxy);
    ...
    config.https.bind.forEach(function (options) {
        counter++;
        var httpsServer = https.createServer(options, httpAppProxy);
        httpsServer.on('connection', tcpConnectionHandler);
        httpsServer.on('upgrade', wsAppProxy);
